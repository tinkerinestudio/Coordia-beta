...
def makeHello(type, value, path):
    ...
    for row in row_reader:
        ...
        if "Feed Rate (mm/s):" in type:
            if "Flow Rate Setting (float)" in row[0]:
                row[1] = str(value)
                
        if "Perimeter Feed Rate Multiplier (ratio):" in type:
            if "Perimeter Flow Rate Multiplier (ratio)" in row[0]:
                row[1] = str(value)
        
        if "Object First Layer Feed Rate Infill Multiplier (ratio):" in type:
            if "Object First Layer Feed Rate Perimeter Multiplier (ratio):" in row[0]:
                row[1] = str(value)
            if "Object First Layer Flow Rate Infill Multiplier (ratio):" in row[0]:
                row[1] = str(value)
            if "Object First Layer Flow Rate Perimeter Multiplier (ratio):" in row[0]:
                row[1] = str(value)
